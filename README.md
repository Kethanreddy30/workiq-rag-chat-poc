...
- an LLM
- Semantic Kernel
- a real JWT authentication provider
- a streaming response implementation
...
The current mock returns plain text.
...
The smoke test sends representative known and unknown questions to the running API.
...
A known benchmark question returns its expected static answer.

## 4. Unknown query

An unknown question returns the controlled fallback.
...
The `.gitignore` is designed to exclude common secret and local-runtime patterns.
...
The mock is intentionally deterministic so that the surrounding system can be validated before introducing the complexity of retrieval, Work IQ, SharePoint, Graph, authentication, and LLM-based evaluation.
